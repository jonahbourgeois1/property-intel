window.PI_STATUS = {
  "as_of": "2026-09-02",
  "as_of_utc": "2026-09-02T00:04:30Z",
  ...
  "sources": {
    "git_log_since": "2026-09-01",
    "iso_week": "2026-W36",
    "changelog": null,
    "context": null,
    "property_intel_v2": "404 from this token; OneDrive v2 docs/ops not mounted",
    "note": "index.html embeds this snapshot so a stale sibling status.js without ## 2026-09-02 cannot hide today"
  },
  "weekly": {
    "label": "2026-W36",
    "start": "2026-08-31",
    ...
    "still_open": [
      {
        ...
        "evidence": "list-cloud-agents returned this cron (bc-7de4187d), 9/01 bc-76a61999, 8/31 bc-f631c354, 8/30 bc-eb481f27, 8/29 bc-8dc1a78f, 8/28 bc-ede78703, 8/27 bc-d4995744, 8/26 bc-45e4c56a, plus internal Search 8/28 transcript for dashboard / Find original ops dashboard schema / Summarize yesterday log bot; desktop/web/local sources returned 0"
      },
      {
        "item": "Daily log PRs #1, #3, #4, #5, #6, and #7 still DRAFT; docs/ops/ not on main",
        "evidence": "gh pr list: #1 cursor/property-intel-daily-log-4f45 DRAFT; #3 cursor/property-intel-daily-log-299e DRAFT; #4 cursor/property-intel-daily-log-0704 DRAFT; #5 cursor/property-intel-daily-log-3f6c DRAFT; #6 cursor/property-intel-daily-log-866d DRAFT; #7 cursor/property-intel-daily-log-0d29 DRAFT; origin/main has no docs/ops"
      }
    ],
    "watchouts": [
      "Apps Script editor-save is not a new deployment; .gs copies in git are not a deploy. golf.gs/config.gs/menu.gs/critique-api.gs/satellite.gs landed on origin/main 2026-09-01 afternoon; satellite.gs changed in 8b1d0d4; MOCKINGBIRD row 277 is the first check after a real deploy.",
      "Public data/*.json is sync-owned. Last sync on origin/main is 187a608 Sync Drone (2026-08-28T14:03:30Z). None of a8b3ceb..8b1d0d4 touched data/.",
    ...
    "pipelines": [
      {
        ...
        "status": "on main",
        "evidence": "origin/main 8b1d0d4; HUB_BUILD 1.8.13; element-review BUILD v6.8.11; golf-review BUILD v1.0.4"
      },
      {
        "id": "sync",
        "name": "Apps Script → GitHub data sync",
        "status": "last sync 2026-08-28",
        "evidence": "187a608 Sync Drone; six Publish Responder Intel ee91147..47def97. Editor-save is not a deploy. No data/ sync after 187a608. a8b3ceb..8b1d0d4 did not touch data/."
      },
      {
        "id": "satellite",
        "name": "Satellite Pass 1 / review / Pass 2",
        "status": "viewers on main; deploy unverified",
        "evidence": "element-review.html BUILD v6.8.11 on HEAD (8b1d0d4 duplicate-instance fix). satellite.gs copy changed in 8b1d0d4; file copy is not a deploy. No satellite.gs deploy evidence 2026-08-29..2026-09-02."
      },
      {
        "id": "golf",
        "name": "Golf review (sheet-only)",
        "status": "on main; Apps Script deploy unverified",
        "evidence": "golf-review.html BUILD v1.0.4 on origin/main 8b1d0d4 (a8b3ceb..8b1d0d4). Seed 150 pins ids 1001-1150; GOLF_MAX_PINS=200 in config.gs copy. golf.gs header: no data/golf/, never merge into pins-catalog.json. .gs copies are not a deploy."
      },
      {
        "id": "plane",
      ...
      },
      {
        ...
        "name": "Live CHEKT",
        "status": "on main",
        "evidence": "064658b hub 1.8.13: property CHEKT under Private; Community Live is an empty slot. live-viewer.html still on origin/main. Unchanged by a8b3ceb..8b1d0d4."
      },
      {
        "id": "photo",
        "name": "Photo intake",
    ...
    "in_progress": [
      {
        ...
        "evidence": "Draft PRs #1, #3, #4, #5, #6, #7. origin/main has no docs/ops."
      },
      {
  ...
  "board": {
    ...
    "summary": "Golf review v1.0.4 and Element Review v6.8.11 on main (duplicate pin instances); hub still 1.8.13; Lane parcels published; photo transport and EagleView access still block scale-out.",
    "as_of": "2026-09-02",
    "last_write": "2026-09-02 00:04 UTC",
    ...
    "kpis": [
      {
        ...
        "evidence": "len(data/satellite/*.json)=506 recount 2026-09-02"
      },
      {
        "label": "Plane-rendered properties",
        "value": "22",
        "sub": "target 44 Highlands roster",
        "evidence": "len(data/plane/*.json)=22 recount 2026-09-02 (was 20 on 2026-08-25)"
      },
      {
        ...
        "items": [
          {
            ...
            "note": "origin/main 064658b; HUB_BUILD 1.8.13; property CHEKT under Private; Community Live empty slot; live-viewer.html on main."
          },
          {
            "status": "done",
            "label": "DONE",
            "title": "Element Review outside-crop pins + Earth",
            "note": "element-review.html BUILD v6.8.11 on HEAD 8b1d0d4 (duplicate catalog pins stay separate instances)."
          },
          {
            "status": "in_progress",
            "label": "IN PROGRESS",
            "title": "Golf review (sheet-only)",
            "note": "golf-review.html BUILD v1.0.4 on origin/main 8b1d0d4. Seed 150 pins ids 1001-1150; GOLF_MAX_PINS=200 in the config.gs copy. .gs copies are not a deploy. No data/golf/."
          },
          {
            "status": "blocked",
            "label": "BLOCKED",
