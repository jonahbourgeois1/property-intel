window.PI_STATUS = {
  "as_of": "2026-09-01",
  "as_of_utc": "2026-09-01T00:05:50Z",
  ...
  "sources": {
    "git_log_since": "2026-08-31",
    "iso_week": "2026-W36",
    "changelog": null,
    "context": null,
    "property_intel_v2": "404 from this token; OneDrive v2 docs/ops not mounted",
    "note": "index.html embeds this snapshot so a stale sibling status.js without ## 2026-09-01 cannot hide today"
  },
  "weekly": {
    "label": "2026-W36",
    "start": "2026-08-31",
    ...
    "still_open": [
      {
        ...
        "evidence": "list-cloud-agents returned this cron (bc-76a61999), 8/31 bc-f631c354, 8/30 bc-eb481f27, 8/29 bc-8dc1a78f, 8/28 bc-ede78703, 8/27 bc-d4995744, 8/26 bc-45e4c56a, plus internal Summarize yesterday log bot / Search 8/28 transcript for dashboard / Find original ops dashboard schema; desktop/web/local sources returned 0"
      },
      {
    ...
    "watchouts": [
      "Apps Script editor-save is not a new deployment; .gs copies in git are not a deploy. No .gs copies and no satellite.gs deploy evidence on 2026-09-01; MOCKINGBIRD row 277 is the first check after a real deploy.",
      "Public data/*.json is sync-owned. Last sync on origin/main is 187a608 Sync Drone (2026-08-28T14:03:30Z). 064658b did not touch data/.",
    ...
    "pipelines": [
      {
        ...
        "status": "on main",
        "evidence": "origin/main 064658b; HUB_BUILD 1.8.13; element-review BUILD v6.8.10"
      },
      {
        "id": "sync",
        "name": "Apps Script → GitHub data sync",
        "status": "last sync 2026-08-28",
        "evidence": "187a608 Sync Drone; six Publish Responder Intel ee91147..47def97. Editor-save is not a deploy. No sync commits after 53346bd besides 064658b (viewer-only)."
      },
      {
        "id": "satellite",
        "name": "Satellite Pass 1 / review / Pass 2",
        "status": "viewers on main; deploy unverified",
        "evidence": "element-review.html BUILD v6.8.10 on HEAD. satellite.gs copies in git are not a deploy. No satellite.gs deploy evidence 2026-08-29..2026-09-01."
      },
      {
        "id": "plane",
      ...
      },
      {
        ...
        "name": "Live CHEKT",
        "status": "on main",
        "evidence": "064658b hub 1.8.13: property CHEKT under Private; Community Live is an empty slot. live-viewer.html still on origin/main."
      },
      {
        "id": "photo",
        "name": "Photo intake",
    ...
    "in_progress": [
      {
        ...
        "evidence": "Draft PRs #1, #3, #4, #5, #6. origin/main has no docs/ops. 8/31 follow-ups on #6 restyled the charcoal/gold board (f69689d)."
      },
      {
  ...
  "board": {
    ...
    "summary": "Hub 1.8.13 on main nests property CHEKT under Private and leaves Community Live empty; Lane parcels published; photo transport and EagleView access still block scale-out.",
    "as_of": "2026-09-01",
    "last_write": "2026-08-31 19:10 CT",
    ...
    "kpis": [
      {
        ...
        "evidence": "len(data/satellite/*.json)=506 recount 2026-09-01"
      },
      {
        "label": "Plane-rendered properties",
        "value": "22",
        "sub": "target 44 Highlands roster",
        "evidence": "len(data/plane/*.json)=22 recount 2026-09-01 (was 20 on 2026-08-25)"
      },
      {
