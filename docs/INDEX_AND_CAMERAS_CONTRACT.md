...
**CHEKT live (gateway, 2026-09-14):** `GET /live` and `/clips` resolve the CHEKT site(s) from the opened property. Passcode is the gate. `PROPERTY_MAP` is an override for collisions and for **multi-site** properties: the value may be `siteId` or `siteId+siteId+siteId`. Gud Cultures hub `1512452d…` (and drone view `b9407057…`) maps to CHEKT 11831+11789+11788. Otherwise the gateway caches CHEKT `/sites` and uniquely matches `site_no` → `account_reference_id`, then index **address**, then **name**. No unique hit → 404. Viewers send `address`/`name` from `data/index/{id}.json` (`gwLiveQuery`). Roster is keyed by `device_id`. Live opens on **All**. Every online camera starts MJPEG on that grid (staggered) and **keeps the socket** when switching parcel tabs or expanding one feed — `.off` only parks the cell (tiny on-screen sliver; `display:none` cancels multipart in Chrome). CHEKT `status: offline` never binds a socket — those hung connections were stealing slots from Parcel 2. Click a feed to expand it; click again, Grid, or Escape returns to the grid. Leaving Live for 3D/2D keeps the hidden iframe until the browser tab is hidden. The dealer key never leaves AWS. Do not put stream URLs or the dealer key in the public repo.
