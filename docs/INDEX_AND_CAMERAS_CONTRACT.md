...
- Nearmap (trial) → Private **Nearmap** navigates to `nearmap-viewer.html?full=1&delivery=…` (full page). **Standard viewer** (top right) returns to this hub. Join is `NEARMAP_DELIVERY_HUB`.
...
**Three-tab shell (hub 1.8.16).** After the gate the bar is **HOME · PRIVATE · COMMUNITY**. Internal stage id for Community stays `public` (`#btn-public`, `?stage=public`). Property CHEKT live is a nested Private substrate (with 3D / 2D / Nearmap / Plugins). Nested **Nearmap** is leave/return (`nearmap-viewer.html?full=1`), not an iframe. Community has its own nested bar: **Map · Live**. Community Live is an empty hub placeholder until neighborhood cameras exist — it must not load `live-viewer.html`.
...
`live` is optional per camera. `live.device` is the CHEKT camera id used to look up `/live` (`device_id`, unique even when several channels share a bridge MAC). Older Jones pins still store the MAC there; the roster indexes unique MACs as well so those keep resolving. `live.name` is the CHEKT roster name when the technician still label does not unique-match (Jones: FRONT DOOR / UPPER DECK / LOWER DECK vs walkway and deck scene labels; Eugene: "Britt's Office Cam View" → `SVC MGR OFFICE`). `joinLiveToPins` copies the roster key from `/live` when `live.device` is already on the pin, else when `live.name` / label / id uniquely equals a roster name, else when a roster name (≥8 chars) appears in exactly one remaining pin. Do not pair leftover cameras by file order. Index does **not** inline this array. Presence is implied: viewer GETs `data/cameras/json/{propertyId}.json` and treats 404 as “no cameras.” `model-viewer.html` loads that same file (walking hub id → view ids → documented sibling hubs, then any `cameras[]` still on the view record) so the Cameras tab and 3D camera pins work even when sync has stripped the array off the view JSON.

**CHEKT live (gateway, 2026-09-21):** `GET /live` and `/clips` open only the CHEKT site ids written for that property in v2 `chekt-sites.json`. Passcode is the gate. The file lists each property's hub id and its view ids, and the CHEKT site ids those ids may open. Gud Cultures is `11831`, `11789`, and `11788` on one row. Each property index carries `chekt_sites`, the CHEKT site ids hardcoded for that hub. Vyanet Bend is `8093` (`BE - VYANET OPERATING GROUP`). Matt King is `4802`. A missing id, or a row with no sites, is 404. Address, name, `site_no`, and the Lambda `PROPERTY_MAP` env are not read. One CHEKT site id may appear on one property only; a duplicate rejects the file and every live request 404s. To move a site, edit that file and redeploy `chekt-viewer-gateway`. Viewers send only `property` (`gwLiveQuery`), the hub id when the index loaded. Roster is keyed by `device_id`. Live opens on **All**. **Every camera with an `mjpeg_url` starts** (staggered, round-robin across parcels) and **keeps the socket** when switching parcel tabs — `.off` only parks the cell (`display:none` cancels multipart in Chrome). Do not cap the wall and do not skip CHEKT `status: offline` as a bind gate. If MJPEG never sends a first byte (Gud Cultures Parcel 3 / site 11788), the wall drops the hung multipart socket and polls `GET /api/v1/snapshots/{mac}/{channel}` with the same JWT — that is how CHEKT's own tiles refresh. Clip thumbnails stay behind only until a live JPEG arrives. Click a feed to expand it; click again, Grid, or Escape returns to the grid. Leaving Live for 3D/2D keeps the hidden iframe until the browser tab is hidden. The dealer key never leaves AWS. Do not put stream URLs or the dealer key in the public repo.

**Gud Cultures cameras (hub `1512452d…`):** git file `data/cameras/json/1512452d9e6e0f1cf0a32255a4392b12.json` — 15 CHEKT cameras (`live.name` + `live.site` 11831/11789/11788). No technician stills. **2026-09-17:** lat/lng/heading/fov/range were estimated from the `gud-cultures-2026-05-29` nadir plus the live wall (placement `estimated-nadir`). The roster is `BLDG 3 *` again (it had briefly been `BLDG 5 *`); labels follow the live names so `joinLiveToPins` matches. cam-12 / cam-14 / cam-15 sit on neighbor taxlot `36061600000103` (south hoop-house field); they show on 2D and may miss the clipped 00105 GLB. Replace with EXIF (`GPSImgDirectionRef` per photo) when GPS stills exist — do not treat these as surveyed. Drone-test sync republishes this file as-is (`camerasFileForSync_`); a later sync will not invent a new layout. 3D GLB is `captures/plane/gud-cultures-2026-05-29/parcels/36061600000105/clipped.glb`.

**3D follow-on (same property, sheet path):** when the mapping download is local, ingest it, set `parcels_ref` by hand after promote, then run drone-test Generate 3D Models / Generate Property Images on a row whose name/address join this hub. Sync This Row publishes `views.drone-test`. Do not paste a CloudFront GLB into git `data/drone/`.

**Estimated pins (2026-09-21):** the same `estimated-nadir` placement, from the parcel nadir only (no live-wall check), is on Matt King `282006d9…`, Butler `305a4423…`, PPS `135df629…`, Albany `757c8b82…`, Sutherlin `f34caa15…`, Roseburg `a06c4a93…`, Myrtle Creek `6d70b38e…`, Grants Pass `bdbd4a55…`, and Central Point `2162ec07…`. Matt King uses the Phoenix Hills nadir. His index `chekt_sites` is `4802` (Driveway, Front Patio, Upstairs Back Patio, Downstairs Patio). Vyanet Bend `744a3639…` is `8093`. Its four pins are estimated on the two metal roofs: suite 110 on the street-side building, suite 120 on the building north of it. Live does not open 4802 for Bend. A drone-test sync keeps `chekt_sites` because the merge only overwrites fields the patch includes. Clear column AF on the Bend row if it still says 11820; the gateway does not read `site_no`. Butler’s render wrote no `nadir.bounds` or `nadir.local`. The 2D outline is `parcel_ring` on the drone-test view: taxlots `1703194304400` (dirt) and `1703194304500` (house) as one ring. The geocode still falls in `4400`; the stored ring is what the map draws. A full drone-test sync keeps an existing `parcel_ring`. The render and GLB stay on `1703194304400`. Camera lat/lng sit on the house. `mx`/`my` are the same spots in the GLB, so the 3D model can place them without bounds. Tracy `2dce25a3…` reads the Jones GPS file `6de88883…` through `CAMERA_HUB_SIBLINGS` (same house).

**Eugene dual hub (hub 1.7.1, GitHub Pages only):** Vyanet Eugene currently has two index files — `8eea64e5…` (site_no / satellite) and `4a484f8c…` (name-hash / drone). Stills and the cameras JSON live only under `4a484f8c…` **on GitHub**. Do not publish a second cameras file for `8eea64e5…` **on GitHub**. `liveAliasIds` walks the sibling so `?property=8eea64e5…` still finds cameras. **On AWS records**, there is one hub: cameras JSON and stills are on `8eea64e5…`. Do not treat this pair as a third hashing rule; merge the GitHub indexes when drone-test join is deployed, then drop the sibling map.
...
**Private** is the mapping workspace. A second bar (**3D / 2D / Nearmap / Live / Plugins**) sits under the main bar and uses the same button chrome. The main bar is 64px (`--bar-h`); the nested bar is 56px (`--pv-bar-h`). 3D is disabled when the index has no model view; 2D is disabled when there is no sat/view mapping; Live is disabled when the property has no live surface; Nearmap is shown when the hub id is in `NEARMAP_DELIVERY_HUB` and **navigates away** to `nearmap-viewer.html?full=1` (not an iframe; `embed=1` unused). Plugins is a hub stub: the 30 mapping verticals (name + description from the Ahart list) say coming-soon until those products ship a plugin page. Embedded `viewer.html` (`embed=1`) shows only **Drone** (the drone-test view) and does not render the 3D Aerial View launch card. Mapping and live iframes stay mounted; the nested swap is z-index only.
