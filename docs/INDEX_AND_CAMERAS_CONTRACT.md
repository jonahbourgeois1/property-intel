...
**Eugene dual hub (hub 1.7.1):** Vyanet Eugene currently has two index files — `8eea64e5…` (site_no / satellite) and `4a484f8c…` (name-hash / drone). Stills live under `data/cameras/images/4a484f8c…/`. Camera metadata is published for both hub ids. `liveAliasIds` also walks the sibling so `?property=8eea64e5…` still finds cameras if only the name-hash file exists. Do not treat this pair as a third hashing rule; merge the indexes when drone-test join is deployed, then drop the sibling map.
...
## 3D pins vs FR / satellite (hub 1.7.0)

Until a property-level `data/pins/{id}.json` exists, the two iframes read different records. The SATELLITE tab shows `data/satellite/{id}.json` (`elements` + `fr.concerns`, Static Maps %). The 3D iframe used to show `data/drone-test/` (or plane/drone) `nadir.element_pins` / `concern_pins`. Those sets can disagree, and drone-test % includes the padded nadir crop — bilinear-mapping a pad pin onto `nadir.local` then `fillGaps` on a ray miss planted sprites in the black void around a clipped GLB.

**Live rule (model-viewer.html):**

1. If this hub has `views.security`, fetch that satellite record, convert FR pins Static Maps % → lat/lng (same `pixelToLatLng` as `viewer.html`) → nadir % via `nadir.bounds`, and use that set on the 3D model and its 2D nadir overlay. Do not clamp. This is how a satellite resync reaches the mesh without a drone-test pin re-run.
2. Otherwise use the 3D view record’s `nadir.element_pins` / `concern_pins`.
3. Place on the mesh only when a downward ray hits **ground** on the GLB (face normal mostly +Z), the point is not on the bounding-box rim, and at least three of four 2 m neighbors also hit ground. A miss, clip-wall hit, or rim sliver is refused (not filled, not moved). Routes may still `fillGaps` across holes; pins must not.
4. Property JSON is fetched with `cache: 'no-store'`. Entering the 3D stage re-reads FR pins so a hub iframe that stayed mounted picks up a resync.
5. Plane/drone-test Apps Script validators drop pins outside the 5–95 image box rather than clamping them onto the edge. The mesh raycast is still the mapping bound — the 5–95 box is the nadir photo, which includes pad.

Phase C (one pin file, 3D supersedes satellite) is still owed. This clause is the interim so the two tabs do not show two pin histories.
...
1. **Now:** stills live in the repo at `data/cameras/images/{propertyId}/cam-NN.jpg`. Metadata is Apps Script–owned: drone-test **Sync This Row** / **Sync drone-test to GitHub** PUTs `data/cameras/json/{propertyId}.json` (paste `shared.gs` + `drone-test.gs`, save, **new deployment version**). Optionally add `views.drone-test` on Jones’s production index via the sheet so `?property=6de88883…` can select that folder. Viewer already walks index views in order `drone-test`, `plane`, `drone`, `security`, `wildfire`.
