...
**Estimated pins (2026-09-21):** the same `estimated-nadir` placement, from the parcel nadir only (no live-wall check), is on Matt King `282006d9…`, Butler `305a4423…`, PPS `135df629…`, Albany `757c8b82…`, Sutherlin `f34caa15…`, Roseburg `a06c4a93…`, Myrtle Creek `6d70b38e…`, Grants Pass `bdbd4a55…`, and Central Point `2162ec07…`. Matt King uses the Phoenix Hills nadir. The four feeds are CHEKT site 4802 (account 11820), filed at 410 SW Columbia; they are on this hub only. Vyanet Bend `744a3639…` has no cameras file. `LIVE_QUERY_OMIT` covers the Bend hub and its drone and drone-test view ids, and any request whose index id is that hub, so those retries cannot send 410 SW Columbia. The gateway also refuses site 4802 unless the property id is the Matt King hub or that hub's own view records. A later drone-test sync restores `site_no` if column AF is still 11820; the omit still blocks it. Clear AF on the Bend row. Butler’s render wrote no `nadir.bounds` or `nadir.local`. The red outline is taxlot `1703194304400` (the empty lot). The house is the next lot east, `1703194304500`. Camera lat/lng sit on that house. `mx`/`my` are the same spots in the GLB, so the 3D model can place them without bounds. Tracy `2dce25a3…` reads the Jones GPS file `6de88883…` through `CAMERA_HUB_SIBLINGS` (same house).
