...
**Three-tab shell (hub 1.8.15).** After the gate the bar is **HOME · PRIVATE · COMMUNITY**. Internal stage id for Community stays `public` (`#btn-public`, `?stage=public`). Property CHEKT live is a nested Private substrate (with 3D / 2D / Nearmap / Plugins). Community has its own nested bar: **Map · Live**. Community Live is an empty hub placeholder until neighborhood cameras exist — it must not load `live-viewer.html`.
