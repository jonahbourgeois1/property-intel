...
**CHEKT live (gateway, 2026-09-14):** `GET /live` and `/clips` resolve the CHEKT site(s) from the opened property. Passcode is the gate. `PROPERTY_MAP` is an override for collisions and for **multi-site** properties: the value may be `siteId` or `siteId+siteId+siteId`. Gud Cultures hub `1512452d…` (and drone view `b9407057…`) maps to CHEKT 11831+11789+11788. Otherwise the gateway caches CHEKT `/sites` and uniquely matches `site_no` → `account_reference_id`, then index **address**, then **name**. No unique hit → 404. Viewers send `address`/`name` from `data/index/{id}.json` (`gwLiveQuery`). Roster is keyed by `device_id`. Live walls start every camera once when Live is first opened. Parcel buttons (All / Parcel 1 / 2 / 3) show or hide cells — they do not restart MJPEG. Click a feed to expand it; click again, Grid, or Escape returns to the grid without restarting streams. Leaving Live for 3D/2D keeps the hidden iframe’s streams until the browser tab is hidden. The dealer key never leaves AWS. Do not put stream URLs or the dealer key in the public repo.

**Gud Cultures cameras (hub `1512452d…`):** git file `data/cameras/json/1512452d9e6e0f1cf0a32255a4392b12.json` — 15 CHEKT names, `live.name` only, no technician stills or lat/lng yet. Pins on the mesh wait for GPS stills (orientation + per-photo `GPSImgDirectionRef`) or a place-on-nadir pass. Do not invent coordinates. 3D GLB attaches later via the drone-test row (`viewer360` / Generate 3D Models), not a hand-edit of the drone JSON. Josephine County is not in the parcel pipeline, so an Eugene-style taxlot clip is blocked until that county is onboarded.
