...
- **Metadata (GitHub):** `data/cameras/images/json/{propertyId}.json`
...
Do not key the image folder on a view-record hash — stills and JSON both use the **index hub id**. `data/cameras/images/` is the container: one folder per property for stills, plus `json/` for metadata. Do not dump one property's JPEGs directly in `images/`.

Viewers fetch `data/cameras/images/json/{id}.json` first, then `data/cameras/json/{id}.json`, then the legacy flat `data/cameras/{id}.json`. 404 = no cameras file.

`data/cameras/images/json/{propertyId}.json` shape:
...
`live` is optional per camera. Index does **not** inline this array. Presence is implied: viewer GETs `data/cameras/images/json/{propertyId}.json` and treats 404 as “no cameras.” `model-viewer.html` loads that same file (walking hub id → view ids, then any `cameras[]` still on the view record) so the Cameras tab and 3D camera pins work even when sync has stripped the array off the view JSON.
...
4. Enable **LIVE FEED** and iframe `live-viewer.html?property={id}&embed=1` when `detectCameras` finds a cameras file (`data/cameras/images/json/{idx.id|propertyId}.json`) or a non-empty `cameras` array on **any** view record, **or** when the property has a 3D view (`hasModel` is a live proxy until cameras files are published — Jones has gateway live with no cameras file). Live does **not** require a GLB.
...
1. **Now (testing):** camera bytes and metadata both live under `data/cameras/images/` — stills at `{propertyId}/cam-NN.jpg`, JSON at `json/{propertyId}.json`. Optionally add `views.drone-test` on Jones’s production index via the sheet so `?property=6de88883…` can select that folder. Viewer already walks index views in order `drone-test`, `plane`, `drone`, `security`, `wildfire`.
2. **Later (automation):** Apps Script PUT `data/cameras/images/json/{propertyId}.json`; normalized stills to tiles `cameras/{propertyId}/cam-NN.jpg` + CloudFront invalidation; fold Tracy extras onto the plane/drone view; drop `views.drone-test`.
