...
**CHEKT live (gateway, 2026-09-14):** `GET /live` and `/clips` resolve the CHEKT site(s) from the opened property. Passcode is the gate. `PROPERTY_MAP` is an override for collisions and for **multi-site** properties: the value may be `siteId` or `siteId+siteId+siteId`. Gud Cultures hub `1512452d…` (and drone view `b9407057…`) maps to CHEKT 11831+11789+11788. Otherwise the gateway caches CHEKT `/sites` and uniquely matches `site_no` → `account_reference_id`, then index **address**, then **name**. No unique hit → 404. Viewers send `address`/`name` from `data/index/{id}.json` (`gwLiveQuery`). Roster is keyed by `device_id`. Live walls start MJPEG only for the visible parcel (default Parcel 1 when there are several sites), and drop `src` on hidden cells so Chrome/CHEKT slots free up. Cap is `MAX_LIVE_MJPEG` (8). CHEKT `status: offline` shows **offline** without waiting on the 18s timeout. Click All to see every cell; only the first eight stream until you pick a parcel. Click a feed to expand it; click again, Grid, or Escape returns to the grid. Leaving Live for 3D/2D keeps the hidden iframe until the browser tab is hidden. The dealer key never leaves AWS. Do not put stream URLs or the dealer key in the public repo.
