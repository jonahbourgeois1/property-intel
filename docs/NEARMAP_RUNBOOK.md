...
**Property Intel → Nearmap Pipeline → Open Nearmap Regions Editor (This Row)** opens `?mode=regions` (v1.7.8): merge/erase vendor regions. Draw pick is a click on the painted region or its centroid — not a 10 m snap to a pin. After a detached stroke, the brush stays on the new region. Revert regions → original, Save publishes the regions diff. Pins are not shown and not touched.
