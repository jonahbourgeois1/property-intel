...
Pins are catalog integer ids as **percent of the Nearmap nadir JPEG** (0,0 = top-left). Viewers unproject with `nadir.bounds`. Refuse non-finite coords; never clamp into the frame. Reviewer regions are published as a diff to GitHub `data/nearmap/edits/{id}.json` (see Regions below). **Do not** write reviewer edits into CloudFront `ai/features.json` or `ai/original/regions.json`.
...
Sheet column W (`Drawn Features`) holds **only a summary** `{ drawn: FC, edits: { url, changed, removed, saved } }` — a cell is capped at 50 000 characters, the regions themselves never go in the sheet. `nearmap-elements` returns `regions_original_url` (derived from AI URL: `/ai/features.json` → `/ai/original/regions.json`) and `regions_edits_url` (set once an edits file has been published). The published record `data/nearmap/{id}.json` carries `regions: { original, edits, edits_saved }`.

Isolation holds: Apps Script is the only GitHub writer (edits go through it), `promote.py` is the only S3 writer (original + seed), Lambda is not involved.
