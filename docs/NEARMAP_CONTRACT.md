...
- `ai/firerisk.json` — optional Property Insights sidecar (wildfireScore, RSI, wind/hail/hurricane, Zone 0/1/2). Macalpine only as of 2026-09-14; omit on other deliveries. `urls.firerisk` on the manifest. Does not replace `ai/edits/regions.json`.
...
**Two open modes.** *Sheet mode* (`site_no=` from the Nearmap sheet, or `property=` from a published record): original and edits both come from CloudFront (`ai/original/` and `ai/edits/`), Save posts the working regions to Apps Script which PUTs them to S3, nothing is reset on reload. *Testing mode* (plain `delivery=` open, no sheet identity): every open starts from `ai/original/regions.json` with no pins; the local edits file and browser backups are ignored and local `ai/edits/regions.json` is reset to the original. `?fresh=0` resumes local edits; `?fresh=1` forces a fresh start even in sheet mode. Review boot (v1.7.18): Maps JS loads immediately without `libraries=geometry`; `original/regions.json` is fetched without a cache-buster (vendor snapshot, cacheable); a fresh testing open does not GET edits; `vert.jpg` is preloaded; Google satellite remains the map type around the opaque nadir (`tilt: 0`). Edits stay `Cache-Control`/query-busted because Save rewrites them.
...
**Review fetch (v1.7.18).** `ai/original/regions.json` is cacheable (no query-string bust). `ai/edits/regions.json` stays uncached (`Cache-Control` / `?_=` bust) because Save rewrites it. Review Maps JS does not load `libraries=geometry`; distances use the local haversine (`distM`). The nadir GroundOverlay stays opaque (`opacity: 1`); Google satellite is the map type so tiles show around the JPEG (`tilt: 0`, no map-type dropdown). A fresh testing open does not GET edits.
...
- **Trial viewer** is `vyanet-viewer.html`. Private **Nearmap** opens `nearmap-viewer.html?full=1` on the whole page; **Standard viewer** (top right) returns to the hub. Join is `NEARMAP_DELIVERY_HUB` in `js/vyanet-viewer/property.js` (Macalpine → Jones `6de88883…`, Columbia → `744a3639…`). Ahartsi’s other five deliveries are registry-only until an **existing** index hub exists — do not invent one. Do **not** hash `site_no` for that join. Still do **not** add `nearmap` to production `VIEW_ORDER` or `syncNow()`.
- **Do not** add Nearmap to `syncNow()`, top-level Sync This Row, or Satellite Pipeline.

## Nearmap viewer (`nearmap-viewer.html`, 2026-09-11)
...
Fed by the Nearmap row and CloudFront: `?delivery=` loads `manifest.json` / `vert.jpg` / regions without waiting on Apps Script; `?site_no=` still asks `nearmap-elements` in the background for pins and Pass 3. `&tiles=` overrides the base for local serve trees. Tabs inside the substrate: **2D** (empty MapType under an opaque nadir GroundOverlay + region polygons for **checked** classes + numbered region-class pins + Pass 3 FR/WF concern pins + lot line), **3D** (Three.js r128 GLB; region **fills** drape the mesh via a heightmap grid so a Building tints the whole footprint on the roof, not a floating cap or a buried ring; outlines drape the surface; pins as sprites; tab disabled when `urls.mesh` is absent), **Obliques** (N/E/S/W + nadir, lightbox). Right rail (v1.1.12) is four **stacked** tabs: **AI layers** (lot line, clip, observed facts, class checkboxes; vegetation layers start off, Lawn Grass on), **First Responder** (Pass 3 FR considerations / recommendations / FR concern pins), **Wildfire** (Nearmap Fire Risk from `ai/firerisk.json` when present, then Pass 3 WF half), **Pins** (region-class names). Map markers follow the rail tab. `?rail=fr|wf|pins` deep-links a pane. If `firerisk.json` exists and the URL has no `rail=`, the Wildfire pane opens first. 2D on that tab draws Zone 0/1/2 buffers (they extend past the taxlot) and the scored roof. Regions come from `ai/edits/regions.json` (no-store; fallback original is cacheable). Sheet **Open Nearmap Viewer (This Row)** opens `nearmap-viewer.html?full=1&site_no=&delivery=`.

**Fire Risk sidecar (v1.1.16).** Optional `ai/firerisk.json` is packed from Feature API `include=all` (wildfireScore, defensibleSpace, RSI, wind/hail/hurricane). Not Pass 3. `pack_ai` still drops score fields from regions; the sidecar holds score + zone rings. Live on Macalpine (`18775-macalpine-loop-bend-or-97702`) **alongside** existing Ahartsi stills, mesh, and reviewer `ai/edits/regions.json` — attach with `promote.py --files`, do not full-promote the scratch folder (that would replace the richer 9-pack regions). Other registry deliveries omit the file; the Wildfire tab then shows Pass 3 only. `zz-scratch-macalpine-firerisk` stays gitignored local scratch.

**Viewer boot (v1.1.14).** Maps JS, `manifest.json`, and `vert.jpg` start together. The nadir GroundOverlay is shown before Apps Script and before the 4.7 MB parcel tile. The map under the opaque nadir is an empty MapType (`#111318`); Google satellite tiles are not requested. Open only `addGeoJson`s classes that are checked (vegetation starts off except Lawn Grass); turning a class on adds it in rAF chunks of 80. Parcel GeoJSON uses the browser cache (it is a published static tile).
...
- **2026-09-14** — Fire Risk sidecar live on Macalpine tiles (`urls.firerisk`). Viewer **v1.1.16**. Other deliveries unchanged.
- **2026-09-14** — Viewer **v1.1.16**: all 14 Feature API packs + `include=all` scores (RSI, wind, hail, hurricane) on the local Macalpine Fire Risk scratch. `include=damage` still 403; NIR/CIR empty at Bend.
- **2026-09-14** — Viewer **v1.1.15**: optional `ai/firerisk.json` on the Wildfire tab (score 1–5 + Zone 0/1/2). Local Macalpine scratch only; do not promote.
- **2026-09-14** — Ahartsi remaining five promoted to tiles + `reference/nearmap.json` (7 deliveries). Hub join table unchanged (Macalpine, Columbia only).
- **2026-09-14** — Review **v1.7.18**: Google satellite is the map type around the opaque nadir again (v1.7.17 empty MapType removed). Parallel boot / cacheable original / chunked draw stay.
- **2026-09-11** — Viewer **v1.1.14**: open does not wait on Apps Script; empty MapType under the nadir; only checked classes are drawn; parcel GeoJSON is cacheable.
