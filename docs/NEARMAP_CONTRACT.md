...
- **Trial viewer** is `vyanet-viewer.html` with a nested Private **Nearmap** tab (`nearmap-viewer.html?embed=1`). Join is `NEARMAP_DELIVERY_HUB` in `js/vyanet-viewer/property.js` (Macalpine → Jones `6de88883…`, Columbia → `744a3639…`). Do **not** hash `site_no` for that join. Still do **not** add `nearmap` to production `VIEW_ORDER` or `syncNow()`.
- **Do not** add Nearmap to `syncNow()`, top-level Sync This Row, or Satellite Pipeline.

## Nearmap viewer (`nearmap-viewer.html`, 2026-09-09)

Trial product surface is **`vyanet-viewer.html`**: same gate, HOME, PRIVATE (3D · 2D · Nearmap · Live · Plugins), COMMUNITY, and GIS facts as every other property. `nearmap-viewer.html` is the Nearmap substrate (iframe `embed=1`). Standalone `?delivery=` for a known trial delivery **redirects** into the hub (`stage=home`). GIS is `data/gis/{hubId}.json` (same file Private 2D/3D already uses). Lot lines are `data/parcels/` with the same Deschutes/Lane 0.07° grid as `viewer.html`.
