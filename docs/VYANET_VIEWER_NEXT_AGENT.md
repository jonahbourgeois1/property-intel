...
**Nearmap trial** (`nearmap-viewer.html` **v1.1.2**, editor `nearmap-review.html` **v1.6.11**):
...
- Nearmap page: 2D / 3D / Obliques, AI layers, lot line, catalog pins. **Clip to taxlot** (default on, v1.1.4) removes everything outside the property line on 2D. 3D region fills **drape the mesh** (~8 cm lift); the 3D mesh itself is still the capture. Off layers are not built until turned on. `embed=1` is unused.
- Editor: paint/erase, holes, gap-close, **Revert changes** vs **Revert to original**. Revert buttons were not clicked in a real browser session.
